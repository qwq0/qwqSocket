...
端到端的通信协议  
对于端到端通信场景中 相比c/s结构计算负载更均衡  
QwQ Peer Socket 协议基于普通 QwQ socket 服务端

-   与 QwQ Socket 服务端的不同点
    -   peer 不发送或接收 告知对端简短名 数据包  
        peer 从对方接收自身事件的简短名  
        事件的简短名总是由调用方决策
    -   简短名必须严格按照标准  
        与 QwQ Socket 仅要求开头字符不同  
        QwQ Peer Socket 简短名必须为合法36进制数  
        最大为32位有符号整数的最大值  
        除 0 以外的简短名不能包含前导零
