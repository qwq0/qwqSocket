**qwq-socket**

***

# qwq-socket

## Classes

- [BinderOperator](classes/BinderOperator.md)
- [EventRule](classes/EventRule.md)
- [QueryError](classes/QueryError.md)
- [QueryTimeoutError](classes/QueryTimeoutError.md)
- [QwQSocketClient](classes/QwQSocketClient.md)
- [QwQSocketServer](classes/QwQSocketServer.md)
- [QwQSocketServerClient](classes/QwQSocketServerClient.md)
- [RuleBinder](classes/RuleBinder.md)
- [RuleType](classes/RuleType.md)

## Functions

- [getTypeDefineByBinder](functions/getTypeDefineByBinder.md)
